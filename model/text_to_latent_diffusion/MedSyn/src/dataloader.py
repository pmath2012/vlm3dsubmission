...
from monai.transforms import (
    Compose,
    LoadImaged,
    EnsureChannelFirstd,  # Updated from AddChanneld
    SpatialPadd,
    ToTensord,
    RandRotated,
    RandZoomd,
...
def get_transforms_aug_seg(shape, crop_shape):
    train_target_transforms = Compose(
        [
            LoadImaged(keys=["image", "lobe", "airway", "vessel", "text"]),
            EnsureChannelFirstd(keys=["image", "lobe", "airway", "vessel"]),  # Updated
...
def get_transforms_seg_multiple():
    train_target_transforms = Compose(
        [
            ...
            EnsureChannelFirstd(keys=["image_sr", "lobe", "airway", "vessel"]),  # Updated

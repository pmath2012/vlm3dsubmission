...
def create_model_and_transform_offline(
    model_name: str,
    modality: str,
    ...
    # Get expected local paths
    config_fpath = os.path.join(local_weights_dir, FILE_DICT_ASSOCIATIONS[model_name]["config"])
    ckpt_fpath = os.path.join(local_weights_dir, FILE_DICT_ASSOCIATIONS[model_name]["ckpt"])

    # Verify both files exist locally
    if not os.path.exists(config_fpath):
        raise FileNotFoundError(f"Missing config file: {config_fpath}")
    if not os.path.exists(ckpt_fpath):
        raise FileNotFoundError(f"Missing checkpoint file: {ckpt_fpath}")

    # Build the model from local files
    model = build_model(model_name, config_fpath, ckpt_fpath)
